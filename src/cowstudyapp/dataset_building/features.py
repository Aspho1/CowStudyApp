...
class AccelerometerFeatures:
    ...
    @staticmethod
    def compute_spectral_features(signals: Dict[str, np.ndarray], 
        ...
        for axis, signal in signals.items():
            ...
            if total_magnitude > 0:
                centroid = float(np.sum(frequencies[:nyquist_idx] * magnitudes[:nyquist_idx]) / 
                            total_magnitude)
            else:
                centroid = 0.0
    ...
    def _compute_window_features(self, window: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            for feature_type in self.config.feature_types:
                ...
                elif feature_type == FeatureType.CORRELATION and self.config.enable_axis_features:
